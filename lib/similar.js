...
function similar(opts) {
  return new Promise(function (resolve, reject) {
    ...
    // Dùng URLSearchParams thay querystring
    ...
    // console.log('Similar Request URL:', similarUrl);
...
function parseSimilarApps(similarObject, opts) {
  ...
  let similarAppsCluster = clusters.filter(cluster =>
    R.path(CLUSTER_MAPPING.title, cluster) === SIMILAR_APPS ||
  ...
  // console.log('Cluster Request URL:', fullClusterUrl);
...
async function processFirstPage(html, opts, savedApps, mappings) {
  ...
  const mapping = {
    ...
    url: {
      path: [10, 4, 2],
      fun: (path) => new URL(path, BASE_URL).toString() // Dùng URL chuẩn web
    },
    icon: [1, 3, 2],
    developer: [14],
    currency: [8, 1, 0, 1],
  ...
  const processedApps = R.map(
    scriptData.extractor(mapping),
    R.path(mappings.apps, html)
  );
...
function getParsedCluster(similarObject) {
  return R.path(CLUSTER_MAPPING.url, similarObject);
}
